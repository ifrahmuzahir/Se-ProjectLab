        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
                <select class="form-control" name="election_id" required>
                    ...
                    <?php
                    $fetchingelections = mysqli_query($db, "SELECT * FROM electiontable where Status = 'Active' ") or die(mysqli_error($db));
                ...
                <select class="form-control" name="party_id" required>
                    ...
                    if ($isanyPartyAdded > 0) {
                        while ($row = mysqli_fetch_assoc($fetchingparty)) {
                            ...
                            ?>
                            <option value="<?php echo $party_id; ?>">
                                <?php echo $party_name; ?>
                            </option>
                            <?php
                        }
                ...
                if ($isanyElectionAdded > 0) {
                    ...
                    while ($row = mysqli_fetch_array($fetchingdata)) {
                        ...
                        <tr>
                            ...
                            <td>
                                <?php echo $row['CandidateDetails'] ?>
                            </td>
                            <td>
                                <?php echo $electionName ?>
                            </td>
                            <td>
                                <?php echo $partyName ?>
                            </td>
    ...
    if ($image_size < 20000000) {
        if (in_array($candidate_pic_type, $allowed_type)) {
            if (move_uploaded_file($cand_temp_pic_name, $candidate_pic)) {
                //insert data into DB
                // echo "<script>location.assign('index.php?addCandidatePage=1&partyid='+ '" . $party_id . "')</script>";
